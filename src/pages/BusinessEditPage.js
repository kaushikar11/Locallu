...
const BusinessEditPage = () => {
  ...
  const loadBusiness = async () => {
    ...
    try {
      ...
      if (id) {
        const businessData = await apiService.getBusinessDetails(id);
  ...
  const handleSaveField = async () => {
    if (!businessId || !editingField) return;

    setUpdating(true);
    try {
      const value = editValue.trim();
      await apiService.updateBusinessDetail(businessId, editingField, value);
      await loadBusiness();
  ...
  const handleImageUpload = async (e) => {
    ...
    setUploading(true);
    try {
      await apiService.updateBusinessProfilePicture(businessId, file);
      await loadBusiness();
    } catch (error) {
  ...
  const organizeFields = (business) => {
    if (!business) return {};
    
    const sections = {
      basic: ['businessName', 'companyDescription', 'productPurpose'],
      contact: ['location', 'contactInfo', 'websiteURL'],
      business: ['industry', 'paymentInfo'],
    };

    const organized = {};
    Object.entries(business).forEach(([key, value]) => {
      if (['uid', 'email', 'photoURL', 'tasks', 'businessImage'].includes(key)) return;
    ...
    return (
      <div style={{...styles.root, alignItems: 'center', justifyContent: 'center'}}>
        <div style={styles.sectionCard} className="apple-card">
          ...
          <Button variant="primary" onClick={() => navigate('/business/form')}>
            Create Profile
          </Button>
        </div>
      </div>
  ...
  const organizedFields = organizeFields(business);
        ...
        >
          <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            <Input
              ...
              type={editingField === 'websiteURL' ? 'url' : 'text'}
              autoFocus
            />
